...
module ysyx_210302_forward_ID_forward_unit(
    ...
    assign  csr_stall  = WB_csr_en & MEM_csr_en & EXE_csr_en & ( WB_rd_addr != 0) & ( WB_rd_addr == ID_rs1_addr | WB_rd_addr == ID_rs2_addr);//由于csr发生的数据冲突产生阻塞的情况应该转发WB级的数据。

    assign  EXE2ID_rs1 =   ( EXE_rd_addr != 0 ) & ( EXE_rd_addr == ID_rs1_addr ) & (!csr_stall);
    assign  EXE2ID_rs2 =   ( EXE_rd_addr != 0 ) & ( EXE_rd_addr == ID_rs2_addr ) & (!csr_stall);//跨一级流水前递

    assign  MEM2ID_rs1  =   ( MEM_rd_addr != 0 ) & ( MEM_rd_addr == ID_rs1_addr ) & ( EXE_rd_addr != ID_rs1_addr)& (!csr_stall);
    assign  MEM2ID_rs2  =   ( MEM_rd_addr != 0 ) & ( MEM_rd_addr == ID_rs2_addr ) & ( EXE_rd_addr != ID_rs2_addr)& (!csr_stall);//跨两级流水前递
...
module ysyx_210302_csr_hazard_unit (  
    ...
    // 跟通用寄存器产生的冒险不同，csr寄存器产生的冒险放在EXE阶段解决，所以当产生冒险的csr指令在MEM，另一指令在EXE时需要阻塞流水线一周期。
endmodule
...
module ysyx_210302_if_axi_rw # (
    ...
    wire [YSYX210302_AXI_ADDR_WIDTH-1:0] axi_addr    = (rw_addr_i < `YSYX210302_MEM_BEG_ADDR) ?{rw_addr_i[31:2], {2{1'b0}}}:
                                            {rw_addr_i[YSYX210302_AXI_ADDR_WIDTH-1:ALIGNED_WIDTH], {ALIGNED_WIDTH{1'b0}}};
            ...
            always @(negedge clock) begin
                if (reset) begin
                    ...
                    if (axi_addr<`YSYX210302_APB_END_ADDR)begin
                        ...
                        if (len[0]) begin
                            axi_w_data_o[YSYX210302_AXI_DATA_WIDTH-1:0] <= axi_w_data_h;
                            axi_w_strb_o <= { mask_h[56],mask_h[48],mask_h[40],mask_h[32],mask_h[24],mask_h[16],mask_h[8],mask_h[0]};
                        end
                        else begin
                            axi_w_data_o[YSYX210302_AXI_DATA_WIDTH-1:0] <= axi_w_data_l;
                            axi_w_strb_o <= { mask_l[56],mask_l[48],mask_l[40],mask_l[32],mask_l[24],mask_l[16],mask_l[8],mask_l[0]} ;
                        end
                    end
                    else if (len == 0) begin
                        axi_w_data_o[0*YSYX210302_AXI_DATA_WIDTH+:YSYX210302_AXI_DATA_WIDTH] <= axi_w_data_l;
                        axi_w_strb_o <= { mask_l[56],mask_l[48],mask_l[40],mask_l[32],mask_l[24],mask_l[16],mask_l[8],mask_l[0]};
                    end
                end
                if ( len == axi_len )
...
module ysyx_210302_if_stage(input wire              clk,
    ...
    begin
        ...
        if ( rst == 1'b1 ) begin
            ...
            if ( jumpb_en ) begin
                if (IF_myinst_addr != ID_offset + ID_myinst_addr ) begin
                    pc <= ID_offset + ID_myinst_addr;
                    IF_fetch_wrong <= 1'b1;
                end
                else IF_fetch_wrong <= 1'b0;
            end
            else if  ( jumpr_en )begin
            ...
            if ( time_intr_pc & pc != WB_mtvec_read & IF_myinst_addr != WB_mtvec_read)begin  //若已经取到了中断处理程序的指令，则不用跳入此分支
                ...
                if ( if_axi_data_read[6:0] ==`YSYX210302_J_TYPE )begin
                    pc  <= pc + { {44{if_axi_data_read[ 19 ]}} , if_axi_data_read[ 19 : 12 ] ,  if_axi_data_read[ 20 ] , if_axi_data_read[ 30 : 21 ] , 1'b0 };     
                    IF_fetch_wrong  <= 1'b0;
                end
                else if ( if_axi_data_read[6:0] ==`YSYX210302_B_TYPE )begin
                    pc  <= pc + 4;
                    IF_fetch_wrong  <= 1'b0;
                end
...
module ysyx_210302_jump_judge(
    ...
    always@( * )
        ...
        else if ( jump_judge_en & jump_forward_ready )
        begin
            case( funct3 )
                `YSYX210302_BEQ:
...
//流水线握手信号 
    ...
    assign  IFID_flush      = IF_intr_refetch | IF_fetch_wrong;
    assign  IDEXE_flush     = load_hazard | IF_intr_refetch;
    assign  EXEMEM_flush    = IF_intr_refetch;
    assign  MEMWB_flush     = IF_intr_refetch;
