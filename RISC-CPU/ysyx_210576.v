...
always @(posedge clock ) begin
    if(reset)begin
      begin_data <= 64'h0; 
    end else if ((cache_d_ar_addr == `YSYX210576_PC_START) && axi_cache_d_r_valid && axi_cache_d_r_last)begin
...
wire jump_equal_tag = ((cache_d_ar_addr == jump_addr_keep) & (cache_d_ar_addr[31] == 1))?1:0;//跳转判断，后四位的偏移位置零
    ...
    //提供的ram例化
    ysyx_210576_S011HD1P_X32Y2D128_BW ysyx_210576_S011HD1P_X32Y2D128_BW0(
            .Q(cache_r_data),
            .clock(clock),
            .CEN(~cache_cen & I_cache_hit),
            .WEN(~wen_ram ),
            .BWEN(bwen),
            .A(I_index),
            .D(cache_w_data)
    ...
    always @(posedge clock) begin
            if(reset == `YSYX210576_RstEnable)begin
                ...
                cache_d_r_last <= 1'b0;    
            end else if((cache_d_ar_addr == `YSYX210576_PC_START + 4) && (axi_cache_d_r_data == begin_data) ) begin
                cache_d_r_valid <= 1'b0;
                cache_d_r_last <= 1'b0; 
...
module ysyx_210576_S011HD1P_X32Y2D128_BW(
    Q, clock, CEN, WEN, BWEN, A, D
);
parameter Bits = 128;
